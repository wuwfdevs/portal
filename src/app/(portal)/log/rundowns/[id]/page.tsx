...
import { markAired, markMissed, moveRundownItem, startBroadcast, submitRundown } from "../../broadcast-actions";
import { createLiveReadItem, fillRundownItem, removeRundownItem, syncRundownBreaks, updateItemOverrides } from "../../rundown-actions";
...
export default async function RundownDetailPage({
  ...
  const [weather, npr] = live
    ? await Promise.all([getCurrentWeatherReading(), getNprEpisodeForProgramOnDate(rundown.program_id, rundown.air_date)])
    : [null, null];
  ...
  const renderFillControls = (brk: RundownBreakDetail) => {
    ...
    return (
      <div className="flex flex-col gap-2 border-t border-line px-5 py-3">
        ...
        <details>
          <summary className="cursor-pointer text-xs font-semibold text-brand-link">Create a one-off live read</summary>
          <form action={createLiveReadItem} className="mt-2 flex flex-col gap-2">
            ...
            <div>
              ...
              <Input id={`live-duration-${brk.id}`} name="duration_seconds" type="number" required min={1} className="w-24" />
            </div>
            <div>
              <Button type="submit" variant="secondary" className="px-2.5 py-1.5 text-xs">
                Add live read
  ...
  const renderMidBroadcastActions = (item: RundownItemDetail, brk: RundownBreakDetail, compact: boolean) => {
    const confirmed = (eventCountByItem.get(item.id) ?? 0) > 0;
    if (!live || confirmed) return null;
    const moveDestinations =
      item.item_kind === "content" && item.contentItem
        ? listValidMoveDestinations(destinationBreaks, brk.id, item.contentItem.content_type as LogContentType, now)
        : [];
      ...
      <div className={`flex flex-wrap gap-2 ${compact ? "mt-2" : "mt-3"}`}>
        ...
        <details className="inline-block">
          ...
          <form action={markMissed} className="mt-2 flex flex-col gap-2 rounded border border-line p-3">
            ...
            <input type="text" name="notes" placeholder="Brief note (optional)" className="rounded border border-line px-3 py-2 text-sm" />
    ...
    <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
      ...
      <div className="min-w-0 flex-1 lg:order-1">
        ...
        <div className="mt-2 mb-1 flex flex-wrap items-center gap-2.5">
          ...
          {timing && <Badge variant={STATE_VARIANT[timing.state]}>{STATE_LABEL[timing.state]}</Badge>}
          {currentBreakId && (
            <a href="#current-break" className="ml-auto text-xs font-semibold text-brand-link">
              Jump to now →
            </a>
          )}
        </div>
        <p className="mb-4 text-xs text-ink-500">
          {rundown.air_date} · {formatStationTimestamp(rundown.shift_start_at)} – {formatStationTimestamp(rundown.shift_end_at)}
        </p>
            ...
            {rundown.breaks.map((brk) => {
              ...
              return (
                <li
                  ...
                  className={isCurrent ? "rounded border-2 border-brand-primary" : "rounded border border-line"}
                >
                  <div className="flex flex-wrap items-center gap-2.5 border-b border-line bg-panel-50 px-5 py-3">
                    ...
                    <span className="font-mono text-sm font-bold text-ink-900">{formatStationTimestamp(brk.scheduled_at)}</span>
                    ...
                    <span className="ml-auto text-xs text-ink-500">
                      Rejoin network by {formatStationTimestamp(brk.network_rejoin_at)} · {brk.available_duration_seconds}s available
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 px-5 pt-3">
                    {statusBadge}
                    {status === "carrying_network" && (
                      <span className="text-xs text-ink-400">Nothing placed — the network feed simply continues. That&apos;s fine.</span>
                    )}
                  </div>
                            ...
                            {isCurrent ? (
                              ...
                              <div className="flex flex-wrap items-start justify-between gap-2">
                                <div className="min-w-0">
                                  ...
                                  {effectiveScript && <p className="mt-1.5 whitespace-pre-wrap text-xs text-ink-700">{effectiveScript}</p>}
                                </div>
                                <span className="shrink-0 font-mono text-xs font-semibold text-ink-900">{itemDuration(item)}s</span>
                              </div>
                            )}
      ...
      <div className="order-first flex w-full shrink-0 flex-col gap-4 lg:order-2 lg:w-80">
        ...
        <div className="rounded border border-line p-4">
          {!live ? (
            <>
              <div className="mb-2 text-xs font-bold uppercase tracking-wide text-ink-400">Status</div>
              <p className="mb-3 text-xs text-ink-500">
                This rundown hasn&apos;t started yet. Starting it marks it in progress and turns on live timing,
                aired/missed/move, and today&apos;s weather and NPR context above.
              </p>
            ...
            <>
              ...
              {unresolvedEntries.length > 0 && (
                <p className="mb-3 text-xs text-ink-500">
                  {unresolvedEntries.length} thing{unresolvedEntries.length === 1 ? "" : "s"} still need an aired, missed, or
                  moved outcome — or content for a required break. Submitting doesn&apos;t require resolving them first.
                </p>
              )}
