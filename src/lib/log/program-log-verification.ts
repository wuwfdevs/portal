...
export function verifyAndResolveEvents(rawEvents: RawEvent[], sourceText: string, knownUnderwriterNames: string[]): VerifiedEvents {
  ...
  // No cross-row deduplication here, deliberately. The model does report
  // one real credit twice (bundled under its avail marker and again as its
  // own cart row), and both readings verify because the text really is
  // there — but "the same credit twice" is a fact about the *break* both
  // readings land in, which only the planner knows (program-log-plan.ts's
  // addCreditToBreak). A same-second rule lived here once and missed every
  // second credit of a break, whose own row prints thirty seconds after
  // the avail's.
