...
WUWF's current export (DAD) looks like this: columns Time | Cart # | Description | Length. A "UW Credit (mm:ss)" row is a break marker with its window. A cart-bearing credit prints as "Underwriter / Copy label" with its cart number and length, and its script prints on the following row — sometimes carried onto the next page's table. A cart-less credit prints its script directly on or right after the marker row; two or more of these can run back to back with nothing separating them, so read the content and count how many distinct advertisers are really there — one advertiser's script that mentions WUWF twice is still one credit. Program starts are rows naming a program with no cart; other fills are rows naming a specific piece of content.
