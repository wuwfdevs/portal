 ...
 * every project, not just their own uploads.
 *
 * Takes no search argument: searching is no longer a filter on this list but
 * a query across transcripts, clips, and projects together — see
 * searchArchive() in lib/transcription/search.ts and design doc §3F.
 */
export async function listProjects(): Promise<TwProject[]> {
  const supabase = await createClient();
  return (
    unwrapRead(
      await supabase.from("tw_projects").select("*").order("created_at", { ascending: false }),
      "the project list",
    ) ?? []
  );
}
