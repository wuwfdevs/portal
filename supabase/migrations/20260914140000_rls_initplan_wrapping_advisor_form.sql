-- Follow-up to 20260914130000_rls_initplan_wrapping.sql: the same policies,
-- in the exact textual form Supabase's advisor recognizes.
--
-- That migration wrapped each caller-only predicate as
-- `(select private.has_log_access(auth.uid()))`, which is the real fix —
-- confirmed on production with EXPLAIN ANALYZE: the predicate became an
-- InitPlan evaluated once per statement, and a 925-row scan dropped from
-- ~51 ms to ~2 ms. But the advisor's `auth_rls_initplan` lint is a text
-- check, not a plan check: it clears a policy only if the literal
-- `( SELECT auth.uid()` appears somewhere in its expression. A predicate
-- wrapped as a whole still contains a bare `auth.uid()` inside the call, so
-- 154 of the 207 policies stayed flagged after the first migration even
-- though every one of them was already running as an InitPlan.
--
-- So each such call is now written `(select private.f((select auth.uid())))`
-- — the outer subquery is still the InitPlan that does the work, the inner
-- one is a no-op the planner folds away, and the lint is satisfied. Only
-- policies whose expression actually changes under that rule are re-altered
-- here; the ones that only compare a column to `auth.uid()` were already in
-- the advisor's preferred form after the first migration. Generated the
-- same way (from pg_policies, reviewed expression by expression); no
-- policy's command, roles, or meaning changes.
--
-- Convention going forward (superseding the first migration's note): write
-- a caller-only predicate as `(select private.<predicate>((select
-- auth.uid())))` and a column comparison as `col = (select auth.uid())`.

alter policy "access_requests_select_admin_only" on public.access_requests
  using ((select private.is_administrator((select auth.uid()))));

alter policy "access_requests_update_admin_only" on public.access_requests
  using ((select private.is_administrator((select auth.uid()))))
  with check ((select private.is_administrator((select auth.uid()))));

alter policy "al_answers_select" on public.al_answers
  using ((select private.has_audience_listening_access((select auth.uid()))));

alter policy "al_answers_update" on public.al_answers
  using ((select private.has_audience_listening_access((select auth.uid()))))
  with check ((select private.has_audience_listening_access((select auth.uid()))));

alter policy "al_media_delete" on storage.objects
  using (((bucket_id = 'audience-listening-media'::text) AND (select private.has_audience_listening_access((select auth.uid())))));

alter policy "al_media_select" on storage.objects
  using (((bucket_id = 'audience-listening-media'::text) AND (select private.has_audience_listening_access((select auth.uid())))));

alter policy "al_queries_delete" on public.al_queries
  using (((select private.has_audience_listening_access((select auth.uid()))) AND (created_by = (select auth.uid()))));

alter policy "al_queries_insert" on public.al_queries
  with check (((select private.has_audience_listening_access((select auth.uid()))) AND (created_by = (select auth.uid()))));
...
alter policy "ap_email_templates_select" on public.ap_email_templates
  using ((select private.has_academic_partnerships_access((select auth.uid()))));

alter policy "ap_email_templates_update" on public.ap_email_templates
  using ((select private.is_academic_partnerships_coordinator((select auth.uid()))))
  with check ((select private.is_academic_partnerships_coordinator((select auth.uid()))));
...
alter policy "ap_submission_events_insert" on public.ap_submission_events
  with check (((select private.has_academic_partnerships_access((select auth.uid()))) AND (actor_id = (select auth.uid()))));
...
alter policy "ap_submissions_update" on public.ap_submissions
  using ((select private.has_academic_partnerships_access((select auth.uid()))))
  with check ((select private.has_academic_partnerships_access((select auth.uid()))));
...
alter policy "audit_events_insert_admin_only" on public.audit_events
  with check (((select private.is_administrator((select auth.uid()))) AND (actor_id = (select auth.uid()))));

alter policy "audit_events_insert_audience_listening" on public.audit_events
  with check (((select private.has_audience_listening_access((select auth.uid()))) AND (actor_id = (select auth.uid()))));

alter policy "audit_events_insert_editorial_editor" on public.audit_events
  with check (((select private.ep_is_editor((select auth.uid()))) AND (actor_id = (select auth.uid()))));

alter policy "audit_events_insert_editorial_inquiry" on public.audit_events
  with check (((select private.has_editorial_inquiry_access((select auth.uid()))) AND (actor_id = (select auth.uid()))));

alter policy "audit_events_insert_log" on public.audit_events
  with check (((select private.has_log_access((select auth.uid()))) AND (actor_id = (select auth.uid()))));

alter policy "audit_events_insert_mcp" on public.audit_events
  with check (((select private.is_active_profile((select auth.uid()))) AND (actor_id = (select auth.uid())) AND (action ~~ 'mcp.%'::text)));

alter policy "audit_events_insert_roadmap_curator" on public.audit_events
  with check (((select private.is_roadmap_curator((select auth.uid()))) AND (actor_id = (select auth.uid()))));

alter policy "audit_events_insert_underwriting" on public.audit_events
  with check (((select private.has_underwriting_access((select auth.uid()))) AND (actor_id = (select auth.uid()))));
...
alter policy "ei_chat_messages_insert" on public.ei_chat_messages
  with check (((select private.has_editorial_inquiry_access((select auth.uid()))) AND ((role = 'assistant'::text) OR (created_by = (select auth.uid())))));

alter policy "ei_chat_messages_select" on public.ei_chat_messages
  using ((select private.has_editorial_inquiry_access((select auth.uid()))));

alter policy "ei_chat_messages_update" on public.ei_chat_messages
  using ((select private.has_editorial_inquiry_access((select auth.uid()))))
  with check ((select private.has_editorial_inquiry_access((select auth.uid()))));

alter policy "ei_context_notes_insert" on public.ei_context_notes
  with check (((select private.has_editorial_inquiry_access((select auth.uid()))) AND (created_by = (select auth.uid()))));
...
alter policy "ep_criteria_select_members" on public.ep_criteria
  using ((select private.ep_has_access((select auth.uid()))));

alter policy "ep_criteria_write_editors" on public.ep_criteria
  using ((select private.ep_is_editor((select auth.uid()))))
  with check ((select private.ep_is_editor((select auth.uid()))));
...
alter policy "ep_pitch_values_write_submitter_or_editor" on public.ep_pitch_values
  using (((select private.ep_is_editor((select auth.uid()))) OR (EXISTS ( SELECT 1
   FROM ep_pitches p
  ...
  with check (((select private.ep_is_editor((select auth.uid()))) OR (EXISTS ( SELECT 1
   FROM ep_pitches p
  WHERE ((p.id = ep_pitch_values.pitch_id) AND (p.submitted_by = (select auth.uid())) AND (p.status = 'open'::text) AND (NOT private.ep_pitch_under_review(p.id)))))));

alter policy "ep_pitches_insert_members" on public.ep_pitches
  with check (((select private.ep_has_access((select auth.uid()))) AND (submitted_by = (select auth.uid()))));
...
alter policy "ep_pitches_update_submitter_or_editor" on public.ep_pitches
  using (((select private.ep_is_editor((select auth.uid()))) OR ((submitted_by = (select auth.uid())) AND (select private.ep_has_access((select auth.uid()))) AND (status = 'open'::text) AND (NOT private.ep_pitch_under_review(id)))))
  with check (((select private.ep_is_editor((select auth.uid()))) OR ((submitted_by = (select auth.uid())) AND (status = 'open'::text))));

alter policy "ep_review_scores_select_visible" on public.ep_review_scores
  using (((select private.ep_has_access((select auth.uid()))) AND private.ep_review_visible(review_id, (select auth.uid()))));

alter policy "ep_reviews_insert_own_while_open" on public.ep_reviews
  with check (((reviewer_id = (select auth.uid())) AND (select private.ep_is_reviewer((select auth.uid()))) AND (private.ep_meeting_status_of(meeting_pitch_id) = 'open'::text)));

alter policy "ep_reviews_select_own_or_revealed" on public.ep_reviews
  using (((select private.ep_has_access((select auth.uid()))) AND ((reviewer_id = (select auth.uid())) OR (private.ep_meeting_status_of(meeting_pitch_id) = ANY (ARRAY['agenda'::text, 'concluded'::text])))));

alter policy "ep_reviews_update_own_while_open" on public.ep_reviews
  using (((reviewer_id = (select auth.uid())) AND (select private.ep_is_reviewer((select auth.uid()))) AND (private.ep_meeting_status_of(meeting_pitch_id) = 'open'::text)))
...
alter policy "ep_story_plans_insert" on public.ep_story_plans
  with check (((select private.ep_is_editor((select auth.uid()))) OR ((reporter_id = (select auth.uid())) AND (EXISTS ( SELECT 1
   FROM ep_pitches p
...
alter policy "ep_story_plans_update" on public.ep_story_plans
  using (((select private.ep_is_editor((select auth.uid()))) OR ((reporter_id = (select auth.uid())) AND (status <> 'approved'::text))))
  with check (((select private.ep_is_editor((select auth.uid()))) OR ((reporter_id = (select auth.uid())) AND (status <> 'approved'::text))));

alter policy "log_broadcast_events_insert" on public.log_broadcast_events
  with check ((select private.has_log_access((select auth.uid()))));
...
alter policy "log_broadcast_events_select_for_underwriting" on public.log_broadcast_events
  using (((select private.has_underwriting_access((select auth.uid()))) AND (EXISTS ( SELECT 1
...
alter policy "log_rundown_items_delete" on public.log_rundown_items
  using (((select private.has_log_access((select auth.uid()))) AND (item_kind <> 'underwriting_credit'::text)));
...
alter policy "profiles_select_own_or_admin" on public.profiles
  using (((id = (select auth.uid())) OR (select private.is_administrator((select auth.uid())))));
...
alter policy "rd_comments_delete" on public.rd_comments
  using (((select private.has_roadmap_access((select auth.uid()))) AND ((author_id = (select auth.uid())) OR (select private.is_roadmap_curator((select auth.uid()))))));

alter policy "rd_comments_insert" on public.rd_comments
  with check (((select private.has_roadmap_access((select auth.uid()))) AND (author_id = (select auth.uid()))));

alter policy "rd_comments_select" on public.rd_comments
  using ((select private.has_roadmap_access((select auth.uid()))));

alter policy "rd_comments_update" on public.rd_comments
  using (((select private.has_roadmap_access((select auth.uid()))) AND (author_id = (select auth.uid()))))
  with check (((select private.has_roadmap_access((select auth.uid()))) AND (author_id = (select auth.uid()))));
...
alter policy "rd_posts_update" on public.rd_posts
  using (((select private.has_roadmap_access((select auth.uid()))) AND ((author_id = (select auth.uid())) OR (select private.is_roadmap_curator((select auth.uid()))) OR (select private.is_administrator((select auth.uid()))))))
  with check (((select private.has_roadmap_access((select auth.uid()))) AND ((author_id = (select auth.uid())) OR (select private.is_roadmap_curator((select auth.uid()))) OR (select private.is_administrator((select auth.uid()))))));

alter policy "rd_votes_insert" on public.rd_votes
  with check (((select private.has_roadmap_access((select auth.uid()))) AND (user_id = (select auth.uid()))));

alter policy "rd_votes_select" on public.rd_votes
  using ((select private.has_roadmap_access((select auth.uid()))));

alter policy "ri_media_delete" on storage.objects
  using (((bucket_id = 'remote-interview-media'::text) AND (select private.has_remote_interview_access((select auth.uid())))));

alter policy "ri_media_select" on storage.objects
  using (((bucket_id = 'remote-interview-media'::text) AND ((select private.has_remote_interview_access((select auth.uid()))) OR private.ri_owns_storage_object(name, (select auth.uid())))));

alter policy "ri_participants_insert" on public.ri_participants
  with check (((select private.has_remote_interview_access((select auth.uid()))) AND (EXISTS ( SELECT 1
   FROM ri_sessions s
...
alter policy "ri_participants_select" on public.ri_participants
  using (((select private.has_remote_interview_access((select auth.uid()))) OR (profile_id = (select auth.uid())) OR (guest_user_id = (select auth.uid()))));

alter policy "ri_participants_update" on public.ri_participants
  using (((select private.has_remote_interview_access((select auth.uid()))) AND (EXISTS ( SELECT 1
   FROM ri_sessions s
  WHERE ((s.id = ri_participants.session_id) AND (s.created_by = (select auth.uid())))))))
  with check (((select private.has_remote_interview_access((select auth.uid()))) AND (EXISTS ( SELECT 1
   FROM ri_sessions s
...
alter policy "ri_session_events_insert" on public.ri_session_events
  with check (((select private.has_remote_interview_access((select auth.uid()))) OR ((participant_id IS NOT NULL) AND private.ri_is_own_participant(participant_id, (select auth.uid())))));

alter policy "ri_session_events_select" on public.ri_session_events
  using ((select private.has_remote_interview_access((select auth.uid()))));

alter policy "ri_sessions_delete" on public.ri_sessions
  using (((select private.has_remote_interview_access((select auth.uid()))) AND (created_by = (select auth.uid()))));

alter policy "ri_sessions_insert" on public.ri_sessions
  with check (((select private.has_remote_interview_access((select auth.uid()))) AND (created_by = (select auth.uid()))));
...
alter policy "ri_track_parts_select" on public.ri_track_parts
  using (((select private.has_remote_interview_access((select auth.uid()))) OR private.ri_is_own_track(track_id, (select auth.uid()))));

alter policy "ri_tracks_select" on public.ri_tracks
  using (((select private.has_remote_interview_access((select auth.uid()))) OR private.ri_is_own_participant(participant_id, (select auth.uid()))));
...
alter policy "sw_sources_delete" on public.sw_sources
  using (((select private.has_transcription_access((select auth.uid()))) AND (created_by = (select auth.uid()))));

alter policy "sw_sources_insert" on public.sw_sources
  with check (((select private.has_transcription_access((select auth.uid()))) AND (created_by = (select auth.uid()))));
...
alter policy "tool_access_select_editorial_members" on public.tool_access
  using (((select private.ep_has_access((select auth.uid()))) AND (tool_id = (select private.ep_tool_id())) AND (revoked_at IS NULL)));

alter policy "tool_access_select_own_or_admin" on public.tool_access
  using (((user_id = (select auth.uid())) OR (select private.is_administrator((select auth.uid())))));
...
alter policy "tools_select_enabled_or_admin" on public.tools
  using (((select private.is_administrator((select auth.uid()))) OR ((enabled = true) AND (EXISTS ( SELECT 1
   FROM profiles
...
alter policy "tools_select_proposed_for_roadmap" on public.tools
  using (((status = 'proposed'::tool_status) AND (select private.has_roadmap_access((select auth.uid())))));
...
alter policy "tw_media_delete" on storage.objects
  using (((bucket_id = 'transcription-media'::text) AND (select private.has_transcription_access((select auth.uid())))));

alter policy "tw_media_insert" on storage.objects
  with check (((bucket_id = 'transcription-media'::text) AND (select private.has_transcription_access((select auth.uid())))));
...
alter policy "underwriting_documents_insert" on storage.objects
  with check (((bucket_id = 'underwriting-documents'::text) AND (select private.has_underwriting_access((select auth.uid())))));

alter policy "underwriting_documents_select" on storage.objects
  using (((bucket_id = 'underwriting-documents'::text) AND (select private.has_underwriting_access((select auth.uid())))));
...
alter policy "uw_contract_schedule_lines_select_for_log" on public.uw_contract_schedule_lines
  using (((select private.has_log_access((select auth.uid()))) AND (EXISTS ( SELECT 1
   FROM uw_scheduled_placements sp
